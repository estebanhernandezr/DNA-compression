...
class STree():
    ...
    def find(self, y):
        ...
        node = self.root
        while True:
            edge = self.edgeLabel(node, node.parent)
    ...
    def edgeLabel(self, node, parent):
...
class _SNode():
    __slots__ = ['_suffix_link', 'transition_links', 'idx', 'new_idx', 'depth', 'parent', 'generalized_idxs']
    ...
    """ Function for traversing the STree in POST-ORDER
    with parameter: a function to perform on the current node"""
    def _traverse(self, f):
        for node in self.transition_links.values():
            node._traverse(f)
        f(self)

    """ Public method to traverse the SuffixTree in post-order,
        following the technical requirements of the article."""
    def traverse_postorder(self, f):
        self._traverse(f)
